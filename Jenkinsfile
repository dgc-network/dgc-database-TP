...
node ('master') {
    ...
    ws("workspace/${env.BUILD_TAG}") {
        ...
        stage ("Build documentation") {
            sh 'docker build . -f docs/dgc-rest-api-build-docs -t dgc-rest-api-build-docs:$ISOLATION_ID'
            sh 'docker run --rm -v $(pwd):/project/dgc-REST-api dgc-rest-api-build-docs:$ISOLATION_ID'
        }

        stage("Archive Build artifacts") {
            sh 'mkdir -p build/debs'
            sh 'docker run -v $(pwd)/build/debs:/build dgc-tp-installed:$ISOLATION_ID bash -c "cp /tmp/dgc-rest-api-tp*.deb /build"'
